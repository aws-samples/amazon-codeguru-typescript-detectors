...
var AWS = require('aws-sdk')
    ...
    function noSqlInjectionNoncompliant() {
        app.get('/api/getallusers', function (req: { body: { params: any } }, res: any) {
            var docClient = new AWS.DynamoDB.DocumentClient({ region: "us-east-1" });
            var params = req.body.params
            // Noncompliant: external user input can be vulnerable to injection attacks.
            docClient.scan(params, function (err: any, data: { Items: { forEach: (arg0: (element: any, index: any, array: any) => void) => void } }) {
                if (err) {
...
var AWS = require('aws-sdk')
    ...
    function noSqlInjectionCompliant() {
        app.get('/api/getallusers', function (req: any, res: { status: (arg0: number) => { (): any; new(): any; json: { (arg0: { status: number; message: string; data: any }): void; new(): any } } }) {
            var docClient = new AWS.DynamoDB.DocumentClient({ region: "us-east-1" });
            var params = {
